...
using xblah_modding_lib.Packages.VPKPackage.Utilities;
using System;
    ...
    public class VpkArchive : PackageArchive, IDisposable
    {
        ...
        private void ReadEntries(string rootPath)
        {
            ...
            while (true)
            {
                ...
                while (true)
                {
                    var directoryName = Reader.ReadNullTermString(Encoding.UTF8).ToLower();
                    ...
                    while (true)
                    {
                        var fileName = Reader.ReadNullTermString(Encoding.UTF8).ToLower();
