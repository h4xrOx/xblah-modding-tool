        ...
        public Dictionary<string, Mod> LoadMods(Launcher launcher)
        {
            ...
            foreach (string path in paths)
            {
                switch (EngineID)
                {
                    case Engine.SOURCE:
                        {
                            xblah_modding_lib.KeyValue gameInfo = xblah_modding_lib.KeyValue.readChunkfile(path + "\\gameinfo.txt");

                            if (gameInfo != null)
                            {
                                string name = gameInfo.getValue("game") + " (" + new DirectoryInfo(path).Name + ")";
                                xblah_modding_lib.KeyValue steamAppIdKV = gameInfo.findChildByKey("steamappid");
                                string modAppId = "-1";
                                ...
                                if (modAppId == "243730" || modAppId == "243750")
                                {
                                    ...
                                    if (searchPaths != null && searchPaths.getChildren() != null)
                                        foreach (xblah_modding_lib.KeyValue searchPath in searchPaths.getChildren())
                                        {
                    ...
                    case Engine.SOURCE2:
                        {
                            xblah_modding_lib.KeyValue gameInfo = xblah_modding_lib.KeyValue.readChunkfile(path + "\\gameinfo.gi");
                            ...
                            if (gameInfo != null)
                            {
                                ...
                                xblah_modding_lib.KeyValue steamAppIdKV = gameInfoBranch.findChildByKey("steamappid");
                                string modAppId = "-1";
                                if (steamAppIdKV != null)
                                    modAppId = steamAppIdKV.getValue();
                    ...
                    case Engine.GOLDSRC:
                        {
                            xblah_modding_lib.KeyValue gameInfo = Config.readChunkfile(path + "\\liblist.gam");
