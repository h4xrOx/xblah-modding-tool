...
namespace SourceSDK.Packages.VPKPackage
{
    internal abstract class VpkReaderBase
    {
        ...
        #region default
        public IEnumerable<VpkDirectory> ReadDirectories(VpkArchive parentArchive, string rootPath)
        {
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    List<PackageFile> entries = ReadEntries(parentArchive, ext, path).ToList();

                    if (path != rootPath && rootPath != "" && !path.StartsWith(rootPath + "/"))
                        entries = new List<PackageFile>();
        ...
        public IEnumerable<PackageFile> ReadEntries(VpkArchive parentArchive, string ext, string path)
        {
            while (true)
            {
