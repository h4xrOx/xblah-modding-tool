...
namespace xblah_modding_tool
{
    public partial class ExpertContentMountForm : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void SearchPathsForm_Load(object sender, EventArgs e)
        {
            ...
            gameinfo = xblah_modding_lib.KeyValue.readChunkfile(gameinfoPath);
            ...
            foreach (xblah_modding_lib.KeyValue searchPath in gameinfo.getChildByKey("filesystem")
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            xblah_modding_lib.KeyValue searchPathsKV = gameinfo.getChildByKey("filesystem").getChildByKey("searchpaths");
            searchPathsKV.clearChildren();
            foreach (String[] searchPath in searchPaths)
            {
                searchPathsKV.addChild(new xblah_modding_lib.KeyValue(searchPath[0], searchPath[1]));
            }

            string path = modPath + "\\gameinfo.txt";

            xblah_modding_lib.KeyValue.writeChunkFile(path, gameinfo, false, new UTF8Encoding(false));
